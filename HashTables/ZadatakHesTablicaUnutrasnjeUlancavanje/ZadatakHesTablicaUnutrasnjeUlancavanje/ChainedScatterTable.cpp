...
void ChainedScatterTable::insert(ChainedScatterObject obj)
{
	...
	if (array[probe].status == 2 && !array[probe].isEqualKey(obj.getKey()))
	{
...
ChainedScatterObject ChainedScatterTable::withdraw(ChainedScatterObject obj)
{
	unsigned int probe = h(obj.getKey());

	if (array[probe].status == 2 && array[probe].isEqualKey(obj.getKey())) {
		ChainedScatterObject to_delete = array[probe];
		...
		unsigned int prev = probe;
		while (probe != -1 && array[probe].status != 2) { // && !array[probe].isEqualKey(obj.getKey())) { // 2 - zauzet
			prev = probe;
			probe = array[probe].next;
		}
		if (probe != -1) {
			...
			std::cout << "Object with key " << obj.getKey() << " not found, can't withdraw." << std::endl;
